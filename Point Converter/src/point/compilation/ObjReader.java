...
public class ObjReader {
    ...
    public ObjReader(String directory) throws FileNotFoundException, Exception{
        ...
        for(int i = 0; i < linebreak.size(); i ++){
            ...
            switch(tokens.nextToken()){
                ...
                /*case "v":{
                    vertices.add(new Vertex(vindex, Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken())));
                    vindex ++;
                    break;
                }*/
                case "vt":{
    ...
    public int findMtlindex(String name){
        for(int i = 0; i < materials.size(); i ++){
            if(materials.get(i).equals(name)) return i;
        }
        return -1;
    }
